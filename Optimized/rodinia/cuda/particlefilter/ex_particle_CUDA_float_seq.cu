...
__device__ double block_reduce_sum(double * __restrict__ buffer, int Nparticles) {
    const int lane = threadIdx.x & 31;
    const int warp_id = threadIdx.x >> 5;
    const int num_warps = (blockDim.x + 31) >> 5;
    const int i = blockIdx.x * blockDim.x + threadIdx.x;

    double sum = (i < Nparticles) ? buffer[threadIdx.x] : 0.0;
    unsigned int active_mask = __ballot_sync(0xffffffff, i < Nparticles);

    // 1) Warp-level reduction via shuffle.
    for (int offset = 16; offset > 0; offset >>= 1) {
        sum += __shfl_down_sync(active_mask, sum, offset);
    }

    // 2) One value per warp in shared memory.
    if (lane == 0) {
        buffer[warp_id] = sum;
    }
    __syncthreads();

    // 3) First warp reduces warp sums to one block sum.
    sum = (warp_id == 0 && lane < num_warps) ? buffer[lane] : 0.0;
    unsigned int warp_sum_mask = __ballot_sync(0xffffffff, warp_id == 0 && lane < num_warps);
    for (int offset = 16; offset > 0; offset >>= 1) {
        sum += __shfl_down_sync(warp_sum_mask, sum, offset);
    }
 ...
 ******************************/
__device__ __forceinline__ double d_randu(int * __restrict__ seed, int index) {
 ...
 * returns a double representing a randomily generated number from a uniform distribution with range [0, 1)
 ******************************/
 __device__ __forceinline__ double2 d_randu_twice(int * __restrict__ seed, int index) {

    double2 randu_twice;
    int M = INT_MAX;
    int A = 1103515245;
    int C = 12345;
    int seed_temp = seed[index];
    int num = A * seed_temp + C;
    randu_twice.x = fabs(seed_temp / ((double) M));
    seed_temp = num % M;
    num = A * seed_temp + C;
    seed[index] = num % M;
    randu_twice.y = fabs(seed[index] / ((double) M));
...
__device__ double d_randn(int * seed, int index) {
    //Box-Muller algortihm
    double pi = 3.14159265358979323846;
    double2 randu_twice = d_randu_twice(seed, index);
    double u = randu_twice.x;
    double v = randu_twice.y;
    double cosine = cos(2 * pi * v);
    double rt = -2 * log(u);
    return sqrt(rt) * cosine;
}

__device__ __forceinline__ double2 d_randn_twice(int * __restrict__ seed, int index) {
    // Keep bitwise behavior consistent with two consecutive d_randn(seed, index) calls.
    int M = INT_MAX;
    int A = 1103515245;
    int C = 12345;
    int seed_temp = seed[index];
    ...
    // First d_randn:
    // u1 = s0 / M, v1 = s2 / M, then seed becomes s2.
    int num = A * seed_temp + C;
    double u1 = fabs(seed_temp / ((double) M));
    seed_temp = num % M;          // s1
    num = A * seed_temp + C;
    seed_temp = num % M;          // s2
    double v1 = fabs(seed_temp / ((double) M));
    double r1 = sqrt(-2.0 * log(u1));

    // Second d_randn:
    // starting from s2, u2 = s2 / M, v2 = s4 / M, final seed is s4.
    num = A * seed_temp + C;
    double u2 = fabs(seed_temp / ((double) M));
    seed_temp = num % M;          // s3
    num = A * seed_temp + C;
    seed_temp = num % M;          // s4
    double v2 = fabs(seed_temp / ((double) M));
    double r2 = sqrt(-2.0 * log(u2));

    seed[index] = seed_temp;

    double2 randn_twice;
    randn_twice.x = r1 * cos(2.0 * pi * v1);
    randn_twice.y = r2 * cos(2.0 * pi * v2);
    return randn_twice;
}
...
__global__ void normalize_weights_kernel(double * __restrict__ weights, int Nparticles, double* __restrict__ partial_sums, double * __restrict__ CDF, double * __restrict__ u, int * __restrict__ seed) {
    int block_id = blockIdx.x;
    int i = blockDim.x * block_id + threadIdx.x;
 ...
 * param12: Nfr
 *****************************/
__global__ void likelihood_kernel(double * __restrict__ arrayX, double * __restrict__ arrayY, double * __restrict__ xj, double * __restrict__ yj, double * __restrict__ CDF, int * __restrict__ ind, int * __restrict__ objxy, double * __restrict__ likelihood, unsigned char * __restrict__ I, double * __restrict__ u, double * __restrict__ weights, int Nparticles, int countOnes, int max_size, int k, int IszY, int Nfr, int * __restrict__ seed, double* __restrict__ partial_sums) {
    int block_id = blockIdx.x;
    int i = blockDim.x * block_id + threadIdx.x;
    int y;
    ...
    double temp_arrayX, temp_arrayY, weight_temp;
    ...
    if (i < Nparticles) {
        ...
        weight_temp = 1 / ((double) (Nparticles));
        // arrayX[i] = arrayX[i] + 1.0 + 5.0 * d_randn(seed, i);
        // arrayY[i] = arrayY[i] - 2.0 + 2.0 * d_randn(seed, i);

        //add noise to the position
        double2 randn_twice = d_randn_twice(seed, i);
        temp_arrayX = xj[i] + 1.0 + 5.0 * randn_twice.x;
        temp_arrayY = yj[i] - 2.0 + 2.0 * randn_twice.y;
        arrayX[i] = temp_arrayX;
        arrayY[i] = temp_arrayY;
    ...
    if (i < Nparticles) {
        for (y = 0; y < countOnes; y++) {
            ...
            indX = dev_round_double(temp_arrayX) + objxy[y * 2 + 1];
            indY = dev_round_double(temp_arrayY) + objxy[y * 2];
            
            int ind_value = abs(indX * IszY * Nfr + indY * Nfr + k);
            ind[i * countOnes + y] = ind_value >= max_size ? 0 : ind_value;
        }
        likelihood[i] = calcLikelihoodSum(I, ind, countOnes, i) / countOnes;
        weights[i] = weight_temp * exp(likelihood[i]); //Donnie Newell - added the missing exponential function call
        
    }

    buffer[threadIdx.x] = (i < Nparticles) ? weights[i] : 0.0;

    __syncthreads();

    double block_sum = block_reduce_sum(buffer, Nparticles);
    if (threadIdx.x == 0) {
