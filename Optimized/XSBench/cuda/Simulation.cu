...
// optimizations made. Following these functions are a number of optimized
// variants, which each deploy a different combination of optimizations
// strategies. By default, XSBench will only run the baseline implementation.
// Optimized variants must be specifically selected using the "-k <optimized
// variant ID>" command line argument.
////////////////////////////////////////////////////////////////////////////////////

unsigned long long
run_event_based_simulation_baseline(Inputs in, SimulationData GSD, int mype) {
  ////////////////////////////////////////////////////////////////////////////////
  ...
  size_t size =  min( (int)(GSD.length_nuclide_grid * sizeof(NuclideGridPoint)),  prop.persistingL2CacheMaxSize ); 
  cudaDeviceSetLimit( cudaLimitPersistingL2CacheSize, size);                                  
  //  set-aside 3/4 of L2 cache for persisting accesses or the max allowed

  size_t window_size = min(prop.accessPolicyMaxWindowSize, (int)(size)); //Select minimum of user defined num_bytes and max window size.

  cudaStreamAttrValue stream_attribute; // Stream level attributes data  structure 
  stream_attribute.accessPolicyWindow.base_ptr  =  reinterpret_cast<void*>(GSD.nuclide_grid);  // Global Memory  data pointer 
  ...
  // xs_lookup_kernel_baseline<<<nblocks, nthreads, 0, stream>>>( in, GSD );
  // gpuErrchk( cudaPeekAtLastError() );
  // gpuErrchk( cudaDeviceSynchronize() );
  
  int shared_mem_size = GSD.length_mats * sizeof(int);
	xs_lookup_kernel_shared_optimization<<<nblocks, nthreads, shared_mem_size, stream>>>(in, GSD);
	gpuErrchk(cudaPeekAtLastError());
	gpuErrchk(cudaDeviceSynchronize());
...
__global__ void xs_lookup_kernel_baseline(Inputs in, SimulationData GSD) {
  ...
  calculate_macro_xs(
      ...
      GSD.concs, // Flattened 2-D array with concentration of each nuclide in
                 // each material
      GSD.unionized_energy_array, // 1-D Unionized energy array
      GSD.index_grid,   // Flattened 2-D grid holding indices into nuclide grid
                        // for each unionized energy level
      GSD.nuclide_grid, // Flattened 2-D grid holding energy levels and XS_data
                        // for all nuclides in simulation
      GSD.mats, // Flattened 2-D array with nuclide indices defining composition
                // of each type of material
      macro_xs_vector, // 1-D array with result of the macroscopic cross section
                       // (5 different reaction channels)
...
__global__ void xs_lookup_kernel_shared_optimization(Inputs in,
                                                     SimulationData GSD) {
  extern __shared__ int shared[];
  // The lookup ID. Used to set the seed, and to store the verification value
  const int i = blockIdx.x * blockDim.x + threadIdx.x;

  int *mats_shared = shared;
  // double *concs_shared = reinterpret_cast<double*>(shared);

  for (int j = threadIdx.x; j < GSD.length_mats; j += blockDim.x) {
    mats_shared[j] = GSD.mats[j];
  }
  // for (int j = threadIdx.x; j < GSD.length_concs; j += blockDim.x) {
  //   concs_shared[j] = GSD.concs[j];
  // }
  __syncthreads();
  if (i >= in.lookups)
    return;
  ...
  calculate_macro_xs(
      ...
      GSD.nuclide_grid, // Flattened 2-D grid holding energy levels and XS_data
                        // for all nuclides in simulation
      mats_shared,      // Flattened 2-D array with nuclide indices defining
                        // composition of each type of material
      macro_xs_vector, // 1-D array with result of the macroscopic cross section
                       // (5 different reaction channels)
      in.grid_type,    // Lookup type (nuclide, hash, or unionized)
      in.hash_bins,    // Number of hash bins used (if using hash lookup type)
...
__device__ void calculate_micro_xs(double p_energy, int nuc, long n_isotopes,
  ...
  } else if (grid_type == UNIONIZED) // Unionized Energy Grid - we already know
  ...
  } else // Hash grid
  {
    ...
    if (p_energy <= e_low)
      ...
      lower = grid_search_nuclide(n_gridpoints, p_energy,
                                  &nuclide_grids[nuc * n_gridpoints], u_low,
                                  u_high);
...
__device__ void
calculate_macro_xs(double p_energy, int mat, long n_isotopes, long n_gridpoints,
                   int *__restrict__ num_nucs, double *__restrict__ concs,
                   double *__restrict__ egrid, int *__restrict__ index_data,
                   NuclideGridPoint *__restrict__ nuclide_grids,
                   int *__restrict__ mats, double *__restrict__ macro_xs_vector,
                   int grid_type, int hash_bins, int max_num_nucs) {
  ...
  for (int j = 0; j < num_nucs[mat]; j++) {
    ...
    calculate_micro_xs(p_energy, p_nuc, n_isotopes, n_gridpoints, egrid,
                       index_data, nuclide_grids, idx, xs_vector, grid_type,
                       hash_bins);
    for (int k = 0; k < 5; k++)
...
__host__ __device__ long grid_search_nuclide(long n, double quarry,
                                             NuclideGridPoint *A, long low,
                                             long high) {
  long lowerLimit = low;
...
// functions, which each deploy a different combination of optimizations
// strategies specific to GPU. By default, XSBench will not run any of these
// variants. They must be specifically selected using the "-k <optimized variant
// ID>" command line argument.

...
// kernel. However, it will be used by better optimization kernels down the
// line.
////////////////////////////////////////////////////////////////////////////////////
unsigned long long run_event_based_simulation_optimization_1(Inputs in,
                                                             SimulationData GSD,
                                                             int mype) {
  const char *optimization_name =
      "Optimization 1 - basic sample/lookup kernel splitting";
...
////////////////////////////////////////////////////////////////////////////////////
unsigned long long run_event_based_simulation_optimization_2(Inputs in,
                                                             SimulationData GSD,
                                                             int mype) {
...
__global__ void xs_lookup_kernel_optimization_2(Inputs in, SimulationData GSD,
                                                int m) {
  // The lookup ID. Used to set the seed, and to store the verification value
  const int i = blockIdx.x * blockDim.x + threadIdx.x;
...
////////////////////////////////////////////////////////////////////////////////////
// This optimization alters Optimization 2. Instead of executing a kernel call
// for ALL different material types, only two different calls are made. One for
// fuel, and one for all the other materials. As the fuel material has by far
// the most isotopes, it takes much longer than the rest.
////////////////////////////////////////////////////////////////////////////////////
unsigned long long run_event_based_simulation_optimization_3(Inputs in,
                                                             SimulationData GSD,
                                                             int mype) {
  const char *optimization_name =
      "Optimization 3 - Fuel or Other Lookup Kernels";
...
__global__ void xs_lookup_kernel_optimization_3(Inputs in, SimulationData GSD,
                                                int is_fuel) {
  // The lookup ID. Used to set the seed, and to store the verification value
  const int i = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  // If this is the fuel kernel, AND this is a fuel lookup, then perform a
  // lookup OR if this is not the fuel kernel, AND this is not a fuel lookup,
  ...
  if (((is_fuel == 1) && (mat == 0)) || ((is_fuel == 0) && (mat != 0))) {
    ...
    calculate_macro_xs(
        ...
        GSD.index_grid, // Flattened 2-D grid holding indices into nuclide grid
                        // for each unionized energy level
        GSD.nuclide_grid, // Flattened 2-D grid holding energy levels and
                          // XS_data for all nuclides in simulation
        GSD.mats,         // Flattened 2-D array with nuclide indices defining
                          // composition of each type of material
...
// hand so that the warps should be densely packed together. This should
// maximize SIMD efficiency of the kernel, but may incur an added cost for the
// sort.
////////////////////////////////////////////////////////////////////////////////////
unsigned long long run_event_based_simulation_optimization_4(Inputs in,
                                                             SimulationData GSD,
                                                             int mype) {
  const char *optimization_name =
      "Optimization 4 - All Material Lookup Kernels + Material Sort";
  ...
  // Count the number of fuel material lookups that need to be performed (fuel
  // id = 0)
  int n_lookups_per_material[12];
  for (int m = 0; m < 12; m++)
    n_lookups_per_material[m] = thrust::count(thrust::device, GSD.mat_samples,
                                              GSD.mat_samples + in.lookups, m);
  ...
  for (int m = 0; m < 12; m++) {
    ...
    xs_lookup_kernel_optimization_4<<<nblocks, nthreads>>>(
...
__global__ void xs_lookup_kernel_optimization_4(Inputs in, SimulationData GSD,
                                                int m, int n_lookups,
                                                int offset) {
  // The lookup ID. Used to set the seed, and to store the verification value
...
////////////////////////////////////////////////////////////////////////////////////
// Optimization 5 -- Kernel Splitting + Fuel/Other Lookups + Fuel/Other
// Partition
////////////////////////////////////////////////////////////////////////////////////
...
unsigned long long run_event_based_simulation_optimization_5(Inputs in,
                                                             SimulationData GSD,
                                                             int mype) {
  const char *optimization_name =
      "Optimization 5 - Fuel/No Fuel Lookup Kernels + Fuel/No Fuel Sort";
  ...
  // Count the number of fuel material lookups that need to be performed (fuel
  // id = 0)
  int n_fuel_lookups = thrust::count(thrust::device, GSD.mat_samples,
                                     GSD.mat_samples + in.lookups, 0);

  // Partition fuel into the first part of the array
  thrust::partition(thrust::device, GSD.mat_samples,
                    GSD.mat_samples + in.lookups, GSD.p_energy_samples,
                    is_mat_fuel());

  // Launch all material kernels individually (asynchronous is allowed)
  nblocks = ceil((double)n_fuel_lookups / (double)nthreads);
  xs_lookup_kernel_optimization_5<<<nblocks, nthreads>>>(in, GSD,
                                                         n_fuel_lookups, 0);

  nblocks = ceil((double)(in.lookups - n_fuel_lookups) / (double)nthreads);
  xs_lookup_kernel_optimization_5<<<nblocks, nthreads>>>(
      in, GSD, in.lookups - n_fuel_lookups, n_fuel_lookups);
...
__global__ void xs_lookup_kernel_optimization_5(Inputs in, SimulationData GSD,
                                                int n_lookups, int offset) {
  // The lookup ID. Used to set the seed, and to store the verification value
  int i = blockIdx.x * blockDim.x + threadIdx.x;
...
//                   + Energy Sort
////////////////////////////////////////////////////////////////////////////////////
// This optimization builds on optimization 4, adding in a second sort by
// energy. It is extremely fast, as now most of the threads within a warp will
// be hitting the same indices in the lookup grids. This greatly reduces thread
// divergence and greatly improves cache efficiency and re-use.
//
// However, it is unlikely that this exact optimization would be possible in a
// real application like OpenMC. One major difference is that particle objects
// are quite large, often having 50+ variable fields, such that sorting them in
// memory becomes rather expensive. Instead, the best possible option would
// probably be to create intermediate indexing (per Hamilton et. al 2019), and
// run the kernels indirectly.
////////////////////////////////////////////////////////////////////////////////////
unsigned long long run_event_based_simulation_optimization_6(Inputs in,
                                                             SimulationData GSD,
                                                             int mype) {
  const char *optimization_name =
      "Optimization 6 - Material & Energy Sorts + Material-specific Kernels";
  ...
  int offset = 0;
  for (int m = 0; m < 12; m++) {
    thrust::sort_by_key(thrust::device, GSD.p_energy_samples + offset,
                        GSD.p_energy_samples + offset +
