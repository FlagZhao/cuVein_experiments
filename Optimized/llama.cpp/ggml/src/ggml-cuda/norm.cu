...
// ============================================================================
// 寄存器缓存优化：所有 norm kernel 都存在多趟循环重复读 global memory 的问题。
// 用寄存器数组缓存每线程处理的元素，消除 pass 间冗余 global load。
//
// 策略：block_size 是编译期已知的模板参数，ncols 是运行时参数。
// niter = ceil(ncols / block_size)，运行时判断是否在寄存器预算内。
// 超出预算时回退到原始逻辑（重复读 global memory）。
// ============================================================================

// 寄存器预算根据 block_size 模板参数确定（模板变量，纯编译期，无函数调用）：
// block_size 小 → dispatch 时 ncols 也小 → 实际 niter 少 → 数组可以小
// block_size 大 → ncols 可以很大 → 数组需要大
template <int block_size>
constexpr int max_reg_iter_1 = block_size <= 64 ? 32 : (block_size <= 256 ? 8 : 32);

template <int block_size>
constexpr int max_reg_iter_2 = block_size <= 64 ? 16 : (block_size <= 256 ? 4 : 16);
...
static __global__ void norm_f32(
    ...
    constexpr int MAXR = max_reg_iter_1<block_size>;
    const int niter = (ncols + block_size - 1) / block_size;
    const bool use_regs = (niter <= MAXR);

    float reg[MAXR];

    float2 mean_var = make_float2(0.0f, 0.0f);

    // Pass 1: 计算 mean 和 var，缓存 x[col] 到寄存器
    if (use_regs) {
#pragma unroll
    ...
    // Pass 2: 归一化——从寄存器读而非重读 global memory
    if (use_regs) {
#pragma unroll
        for (int i = 0; i < MAXR; ++i) {
            const int col = i * block_size + tid;
            if (col >= ncols) return;
            dst[col] = (reg[i] - mean) * inv_std;
        }
    } else {
        for (int col = tid; col < ncols; col += block_size) {
            dst[col] = (x[col] - mean) * inv_std;
...
static __global__ void group_norm_f32(const float * x, float * dst, const int group_size, const int ne_elements, const float eps) {
    ...
    // group_norm 有三趟：求 mean、求 variance（写中间值到 dst）、最终 scale
    // 原始代码 pass2 写 dst[j] = xi, pass3 再读 dst[j]，冗余 global load+store
    // 优化：寄存器缓存 x[j]-mean，pass3 直接从寄存器读

    constexpr int MAXR = max_reg_iter_1<block_size>;
    const int niter = (end - start + block_size - 1) / block_size;
    const bool use_regs = (niter <= MAXR);
...
#pragma unroll
        for (int i = 0; i < MAXR; ++i) {
            const int j = start + i * block_size;
            if (j >= end) break;
            reg[i] = x[j];
            tmp += x[j];
        }
    ...
    // Pass 2: 计算 (x - mean) 和 variance
    if (use_regs) {
#pragma unroll
        for (int i = 0; i < MAXR; ++i) {
            const int j = start + i * block_size;
            if (j >= end) break;
            const float xi = reg[i] - mean;
            reg[i] = xi;
            tmp += xi * xi;
        }
    } else {
...
static __global__ void rms_norm_f32(const float * x,
    ...
    // Pass 1: 求 sum of squares，缓存 x[col]
    float tmp = 0.0f;
...
#pragma unroll
        for (int i = 0; i < MAXR; ++i) {
            ...
            if constexpr (do_multiply && do_add) {
                const int mul_col = fastmodulo(col, mul_ncols_packed);
                const int add_col = fastmodulo(col, add_ncols_packed);
                dst[col]          = scale * reg[i] * mul[mul_col] + add[add_col];
            } else if constexpr (do_multiply) {
                const int mul_col = fastmodulo(col, mul_ncols_packed);
                dst[col]          = scale * reg[i] * mul[mul_col];
            } else {
                dst[col] = scale * reg[i];
            }
        }
...
static __global__ void rms_norm_back_f32(
    ...
    constexpr int MAXR = max_reg_iter_2<block_size>;
    const int niter = (ncols + block_size - 1) / block_size;
    const bool use_regs = (niter <= MAXR);
    ...
    // Pass 1: 求 sum_xx 和 sum_xg，缓存 xf 和 grad
    float sum_xx = 0.0f;
    float sum_xg = 0.0f;
...
#pragma unroll
    ...
    // Pass 2: 从寄存器读 xf 和 grad
    if (use_regs) {
#pragma unroll
        for (int i = 0; i < MAXR; ++i) {
            const int col = i * block_size + tid;
            if (col >= ncols) return;
            dst[col] = scale_grad*reg_grad[i] + scale_x*reg_xf[i];
        }
    } else {
        for (int col = tid; col < ncols; col += block_size) {
            dst[col] = scale_grad*grad[col] + scale_x*xf[col];
